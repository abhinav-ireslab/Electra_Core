...
public interface NotificationRepository extends CrudRepository<Notification, Integer>{
	...
	@Query(value = "SELECT c FROM Notification c WHERE c.status =:status AND c.correlationId =:correlationId ORDER BY c.createdDate DESC")
	public List<Notification> findAllByCorrelationId(@Param ("status") boolean status,@Param ("correlationId") String correlationId);
